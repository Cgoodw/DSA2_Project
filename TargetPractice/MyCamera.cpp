...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	vector3 direction = GetTarget() - GetPosition();
	direction = glm::normalize(direction);

	m_v3Position += a_fDistance * direction;
	m_v3Target += a_fDistance * direction;
	m_v3Above += a_fDistance * direction;
	//old
...
void MyCamera::MoveVertical(float a_fDistance)
{
	...
	vector3 direction = GetAbove() - GetPosition();
	direction = glm::normalize(direction);
...
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	//vector3 rightVector = vector3(sin(GetTarget().x), GetTarget().y, cos(GetTarget().z));
	vector3 direction = GetTarget() - GetPosition();
